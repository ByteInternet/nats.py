...
Mark a release for project, ensure commits in main are in debian changelog.
...
VERSION=$(git tag -l | grep -v hypernode | tail -n1 | cut -dv -f2)
...
export DEBFULLNAME="${DEBFULLNAME:-Hypernode Tech Team}"
export DEBEMAIL="${DEBMAIL:-tech@hypernode.com}"
...
deb_changelog_updated() {
    ...
    gbp dch --debian-tag="%(version)s" --debian-branch=main
...
mark_release() {
    ...
    gbp dch --debian-tag="%(version)s" --new-version=$VERSION --debian-branch=main
    git add $CHANGELOG
}
